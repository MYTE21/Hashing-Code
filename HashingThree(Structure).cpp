...
void insert(int key, string name, int age, double cgpa)
{
...
void display()
{
    for(int i=0;i<SIZE;i++)
    {
        if(dataList[i] != NULL)
        {
            printf(" (%d -> %s, %d, %f) ", dataList[i]->key, dataList[i]->data->name.c_str(), dataList[i]->data->age, dataList[i]->data->cgpa) ;
        }
        else printf(" ~~ ") ;
    }
    printf("\n") ;
...
int main()
{
    ...
    item != NULL ? printf("Element found : %d -> (%s, %d, %f)\n", item->key, item->data->name.c_str(), item->data->age, item->data->cgpa)
